...
export function formatScore(score: any): string {
  const homeTeam = score.competitions[0].competitors.find((c: any) => c.homeAway === 'home');
  const awayTeam = score.competitions[0].competitors.find((c: any) => c.homeAway === 'away');
  ...
  const broadcasts = score.competitions[0].broadcasts?.map((b: any) => b.names.join('/')).join(', ');
  ...
  if (venue) content += ` at ${venue}`;
  content += '.\n\n';
  
  if (broadcasts) content += `You can watch this game on ${broadcasts}.\n\n`;
  ...
  if (status === 'Final') {
    ...
    if (time) content += ` (${time} remaining)`;
  }
  content += '\n\n---\n\n';
  return content;
}

export function formatNews(article: any): string {
  ...
  let details = [];
  if (byline) details.push(`Written by ${byline}`);
  details.push(`Published ${format(new Date(article.published), 'MMMM d')}`);
  if (isPremium) details.push('This is a premium article');
...
export function formatTeam(team: any): string {
  ...
  if (conference || division) {
    ...
    if (conference) content += `they play in the ${conference}`;
    if (conference && division) content += ' and more specifically';
    if (division) content += ` in the ${division}`;
    content += '.\n\n';
  }
  ...
  if (team.color || team.alternateColor) {
    ...
    let colors = [];
    if (team.color) colors.push(team.color);
    if (team.alternateColor) colors.push(team.alternateColor);
...
export function formatRanking(ranking: any): string {
...
export function formatPlayerStats(stats: any): string {
  ...
  if (stats.splits?.recent) {
    ...
    stats.splits.recent.forEach((game: any) => {
...
export function formatPlayerProps(props: any): string {
  let content = '';
...
export function formatStandings(standings: any): string {
  let content = '';
