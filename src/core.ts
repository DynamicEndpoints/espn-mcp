...
class LazyCache<T> {
  ...
  private activeLoads = new Map<string, Promise<T>>();
...
class LazyESPNService {
  ...
  private getTTL(toolName: string): number {
    ...
    return this.cacheTTL[sport as keyof typeof this.cacheTTL] || this.cacheTTL.default;
  }

  private extractSport(toolName: string): string {
    if (toolName.includes('nfl')) return 'nfl';
    if (toolName.includes('nba')) return 'nba';
    if (toolName.includes('mlb')) return 'mlb';
    if (toolName.includes('nhl')) return 'nhl';
    if (toolName.includes('college_football') || toolName.includes('cfb')) return 'cfb';
    if (toolName.includes('soccer')) return 'soccer';
    return 'default';
  }
  ...
  async callTool(name: string, args: any): Promise<any> {
    ...
    return this.cache.get(
      cacheKey,
      () => this.fetchESPNData(name, args),
      ttl
    );
  }

  private async fetchESPNData(name: string, args: any): Promise<any> {
    let url = '';
  ...
  private formatResponse(toolName: string, data: any): string {
    ...
    try {
      if (toolName.includes('scores') && data.events) {
        ...
        content += '## Teams\n\n';
        data.sports[0].leagues[0].teams.slice(0, 32).forEach((teamData: any) => {
        ...
        if (data.groups) {
          data.groups.forEach((group: any) => {
            ...
            group.standings?.entries?.forEach((entry: any) => {
              ...
              const wins = entry.stats?.find((s: any) => s.name === 'wins')?.value || 0;
              const losses = entry.stats?.find((s: any) => s.name === 'losses')?.value || 0;
              const pct = entry.stats?.find((s: any) => s.name === 'winPercent')?.value || '0.000';
        ...
        content += '## Rankings\n\n';
        data.rankings[0].ranks.slice(0, 25).forEach((rank: any) => {
  ...
  private getLeagueFromTool(toolName: string): string {
    if (toolName.includes('nfl')) return 'NFL';
    if (toolName.includes('nba')) return 'NBA';
    if (toolName.includes('mlb')) return 'MLB';
    if (toolName.includes('nhl')) return 'NHL';
    if (toolName.includes('college_football')) return 'College Football';
    return 'Sports';
  }

  private getTypeFromTool(toolName: string): string {
    if (toolName.includes('scores')) return 'Scores';
    if (toolName.includes('news')) return 'News';
    if (toolName.includes('teams')) return 'Teams';
    if (toolName.includes('standings')) return 'Standings';
    if (toolName.includes('rankings')) return 'Rankings';
    return 'Data';
  }
...
export function createTransport(type: 'stdio' = 'stdio') {
  switch (type) {
    case 'stdio':
