      ...
      try {
        ...
        switch (name) {
          ...
          case 'get_nfl_standings':
            ...
            response.data.standings.forEach((div: any) => {
                ...
                content += formatStandings({
                  ...
                  teams: group.standings.entries.map((entry: any) => ({
                    ...
                    record: {
                      ...
                      percentage: entry.stats.find((s: any) => s.name === 'winPercent')?.value || 0,
                      gamesBack: entry.stats.find((s: any) => s.name === 'gamesBack')?.value || '-',
                      last10: `${entry.stats.find((s: any) => s.name === 'last10Wins')?.value || 0}-${entry.stats.find((s: any) => s.name === 'last10Losses')?.value || 0}`,
                      streak: entry.stats.find((s: any) => s.name === 'streak')?.value || '-'
                    }
                  }))
                });
              });
          ...
          case 'get_nfl_player_props':
            ...
            content += response.data.props.map((prop: any) => formatPlayerProps(prop)).join('\n');
            return {
              content: [
                {
                  type: 'text',
        ...
        if (name.includes('scores')) {
          content += response.data.events.map((event: any) => formatScore(event)).join('\n\n');
        } else if (name.includes('news')) {
          content += response.data.articles.map((article: any) => formatNews(article)).join('\n\n');
